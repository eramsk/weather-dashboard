function initPage() {
  ...
  function currentWeather(city) {
    ...
    $.ajax({
      ...
      // Current City Name
      var currentCity = $(
        " <h3 id='city-name' class='city-name' align-middle>"
      ).text(response.name);
      ...
      var weatherIcon = $("<img id='weatherImg'>").attr(
        "src",
      ...
      var currentTemp = $("<p id='temperature'></p>").text(
        "Temperature " + tempC + " ℃"
      );
      todayWeather.append(currentTemp);

      //Wind
      var currentWind = $("<p id='wind-speed'></p>").text(
        "Wind Speed " + response.wind.speed + " km/h"
      );
      todayWeather.append(currentWind);

      //Humidity
      var currentHumidty = $("<p id='humidity'></p>").text(
      ...
      for (var i = 0; i < forecastDays.length; i++) {
        ...
        var forecastIcon = $("<img>").attr(
          ...
          "https://openweathermap.org/img/wn/" +
            data.list[dataIndex].weather[0].icon +
            "@2x.png"
        );
        ...
        //Forecast Temp
        var forecastTemp = $("<p>")
          .text("Temperature " + data.list[dataIndex].main.temp + "\u2103")
          .addClass("card-text");
        forecastTemp.css("color", "#05D9FF");

        //Forecast Wind Speed
        var forecastWind = $("<p>")
          .text("Wind Speed " + data.list[dataIndex].wind.speed + " km/h")
          .addClass("card-text");
        forecastWind.css("color", "#05D9FF");

        // Forecast Humidity
        var forecastHumidity = $("<p>")
          .text("Humidity " + data.list[dataIndex].main.humidity + " %")
          .addClass("card-text");
